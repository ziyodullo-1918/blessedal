import { createFileRoute, Link } from "@tanstack/react-router";
import { RequireAuth } from "@/components/RequireAuth";
import { useEffect, useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { listStagesByDept } from "@/lib/factory/data";
import { listLaserAttendance, laserSalaryReport, type LaserSalaryRow } from "@/lib/factory/laser";
import { Flame, Users, Coins, ClipboardList, BarChart3, CalendarCheck } from "lucide-react";

export const Route = createFileRoute("/factory/laser/")({
  component: () => <RequireAuth><Page /></RequireAuth>,
});

function isoDay(d: Date) { return d.toISOString().slice(0, 10); }

function Page() {
  const [active, setActive] = useState(0);
  const [todayAtt, setTodayAtt] = useState(0);
  const [report, setReport] = useState<LaserSalaryRow[]>([]);

  useEffect(() => {
    const today = new Date();
    const first = isoDay(new Date(today.getFullYear(), today.getMonth(), 1));
    const last = isoDay(new Date(today.getFullYear(), today.getMonth() + 1, 0));
    const t = isoDay(today);
    (async () => {
      const stages = await listStagesByDept("laser");
      setActive(stages.filter((s) => s.status !== "completed").length);
      const att = await listLaserAttendance(t, t);
      setTodayAtt(att.length);
      setReport(await laserSalaryReport(first, last));
    })();
  }, []);

  const monthTotal = report.reduce((a, r) => a + Number(r.total_amount), 0);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-display tracking-tight flex items-center gap-2"><Flame className="size-7 text-orange-400" />Lazer bo'limi</h1>
        <p className="text-sm text-muted-foreground">Kunbay ish formati — boshqaruv paneli</p>
      </div>

      <div className="grid gap-3 md:grid-cols-3">
        <Kpi label="Faol topshiriqlar" value={active} icon={<ClipboardList className="size-5" />} />
        <Kpi label="Bugungi davomat" value={todayAtt} icon={<CalendarCheck className="size-5" />} />
        <Kpi label="Oylik xarajat" value={`${monthTotal.toLocaleString()} so'm`} icon={<Coins className="size-5" />} />
      </div>

      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <NavCard to="/factory/laser/workers" icon={<Users className="size-5" />} title="Ishchilar" desc="Bo'lim ishchilarini boshqarish" />
        <NavCard to="/factory/laser/rates" icon={<Coins className="size-5" />} title="Kunlik stavka" desc="Default va shaxsiy stavkalar" />
        <NavCard to="/factory/laser/attendance" icon={<CalendarCheck className="size-5" />} title="Davomat" desc="Kunlik ish belgilash" />
        <NavCard to="/factory/laser/tasks" icon={<ClipboardList className="size-5" />} title="Topshiriqlar" desc="Faol vazifalar" />
        <NavCard to="/factory/laser/report" icon={<BarChart3 className="size-5" />} title="Oylik hisobot" desc="Har ishchi bo'yicha" />
      </div>
    </div>
  );
}

function Kpi({ label, value, icon }: { label: string; value: React.ReactNode; icon: React.ReactNode }) {
  return (
    <Card>
      <CardContent className="flex items-center justify-between py-5">
        <div>
          <div className="text-xs uppercase text-muted-foreground tracking-wider">{label}</div>
          <div className="text-2xl font-display mt-1">{value}</div>
        </div>
        <div className="text-muted-foreground">{icon}</div>
      </CardContent>
    </Card>
  );
}

function NavCard({ to, icon, title, desc }: { to: string; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <Link to={to}>
      <Card className="hover:bg-accent/40 transition-colors h-full">
        <CardHeader className="pb-2">
          <div className="text-muted-foreground">{icon}</div>
          <CardTitle className="text-base">{title}</CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">{desc}</CardContent>
      </Card>
    </Link>
  );
}
