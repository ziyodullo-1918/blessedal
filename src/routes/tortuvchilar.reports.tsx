...
export const Route = createFileRoute("/tortuvchilar/reports")({
  ...
  const { data: rows, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        ...
        .select(
          "id, quantity, unit_price, total, work_date, worker_id, product_id, pullers_workers(name, worker_code), pullers_products(name, pullers_pullers_categories(name))",
        )
        .gte("work_date", from)
        .lte("work_date", to)
